...
def print_meta(obj):
    ...
    def dump_section(name, start, end, desc):
        hex_dump = " ".join(
            f"{raw[i]:02x}" for i in range(start, min(end, size)))
    ...
    def payload(start=16):
        if isinstance(obj, int):
            ...
            dump_section("ob_item", start + 16, start +
                         24, "Pointer to item array")
            ...
            dump_section("ptr", start, start + 4, "foreign instance pointer")
